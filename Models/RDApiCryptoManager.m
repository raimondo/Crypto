...
//https://api.mybitx.com/api/1/ticker?pair=ETHXBT
...
+(void)fetchLunoRates:(NSString*)pair
{
   ...
   // @"https://api.mybitx.com/api/1/ticker?pair=XBTZAR"
    ...
    NSString*  newString =   [NSString stringWithFormat:@"https://api.mybitx.com/api/1/ticker?pair=%@",pair];
                                                ...
                                                completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
                                                    if (data) {
                                                        ...
                                                        if (json) {
                                                            ...
                                                            RDLunoRate *cryto = [RDLunoRate parseLunoRatesDTOToModel:json];
                                                                   [self postLunoRate:cryto];
    ...
    NSURLSessionDataTask *dataTask = [session dataTaskWithRequest:request
                                                completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
                                                    if (data) {
                                                        ...
                                                        if (json) {
                                                            NSArray *crytos = json;
                                                            NSMutableDictionary * marketCapDic = [[NSMutableDictionary alloc]init];
                                                            
                                                            for (NSDictionary *cryptoDic in crytos)
                                                            {
                                                                RDMarketCap *cryto = [RDMarketCap parseMarketCapDTOToModel:cryptoDic];
                                                                [marketCapDic setObject:cryto forKey:cryto.symbol];
                                                            }
                                                           
                                                            [self postMarketCap:marketCapDic];
                                                        }
                                                    }
                                                }];
    [dataTask resume];
...
+ (void)postLunoRate:(RDLunoRate*)rate
{
    dispatch_queue_t backgroundQueue = dispatch_queue_create("com.mycompany.myqueue", 0);
    dispatch_async(backgroundQueue, ^{
        dispatch_async(dispatch_get_main_queue(), ^{
            [[NSNotificationCenter defaultCenter] postNotificationName:@"lunoRate" object:nil userInfo:@{rate.pair: rate}];
        });
    });
}
